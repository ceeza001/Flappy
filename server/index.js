...
bot.on('callback_query', async (ctx) => {
  try {
    ...
    // Check if the game short name matches
    if (query.game_short_name !== gameName) {
      // Send a text message if the game is not available
      await ctx.answerCallbackQuery({
        text: `Sorry, '${query.game_short_name}' is not available.`,
        show_alert: true // Optional: show an alert popup instead of a small notification
      });
    } else {
